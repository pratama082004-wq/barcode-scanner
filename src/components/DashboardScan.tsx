          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              ...
              <div className="flex space-x-2">
                ...
                {scannedItems.length > 0 && <button onClick={() => setResetConfirmOpen(true)} className="bg-red-50 hover:bg-red-100 text-red-600 text-sm font-medium py-2 px-4 rounded-lg border border-red-200">Reset Semua Data</button>}
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse min-w-[400px]">
                ...
                <tbody className="text-sm">
                  {scannedItems.length === 0 ? (
                    <tr><td colSpan={4} className="p-8 text-center text-gray-400">Belum ada data pindaian barcode.</td></tr>
                  ) : (
                    scannedItems.map((item, index) => (
                      <tr key={item.id} className="border-b last:border-0 hover:bg-blue-50/30 group">
                        ...
                        <td className="p-4 text-center">
                          <button onClick={() => setDeleteConfirm({ isOpen: true, itemId: item.id, barcodeId: item.barcode_id })} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-100 rounded-lg opacity-100 md:opacity-0 md:group-hover:opacity-100"><TrashIcon className="w-5 h-5"/></button>
                        </td>
                      </tr>
                    ))
                  )}
